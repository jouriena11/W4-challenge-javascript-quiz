...
var answerBtnDelegate
//  = document.querySelector(".answer-button")[0].getElementByTagName("button");
...
function startQuiz() {
    ...
    // answerQuestion();
    answerBtnDelegate = document.querySelector(".answer-button")[0].getElementByTagName("button");
}
...
function nextQuestion() {
    quiz.children[index].style.display = "none";
    quiz.children[nextIndex].style.display = "block";
...
function assignClick() {
    for (var i = 0; i < answerBtnDelegate.length; i++) {
    answerBtnDelegate[i].addEventListener("click", function(event) {
    ...
    if (target.matches("button")) {
        ...
        answerBtnDelegate[i].parentElement.appendChild(commentAppend);
        }
